<template>
    <div class="detail-action">
      ...
      <template v-if="order.orderState === 2">
        <XtxButton type="primary" size="small" ><a @click="$router.push(`/member/checkout?orderId=${order.id}`)" href="javascript:;">再次购买</a></XtxButton>
      </template>
      <!-- 待收货 -->
      <template v-if="order.orderState === 3">
        <XtxButton type="primary" size="small" @click="handlerConfirm(order)">确认收货</XtxButton>
        <XtxButton type="plain" size="small"><a @click="$router.push(`/member/checkout?orderId=${order.id}`)" href="javascript:;">再次购买</a></XtxButton>
      </template>
      <!-- 待评价 -->
